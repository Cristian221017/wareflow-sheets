-- Corrigir função get_current_user_dashboard para não contar NFs em viagem ou entregues como armazenadas
CREATE OR REPLACE FUNCTION public.get_current_user_dashboard()
 RETURNS TABLE(user_type text, transportadora_id uuid, cliente_id uuid, solicitacoes_pendentes bigint, nfs_armazenadas bigint, nfs_confirmadas bigint, nfs_em_viagem bigint, nfs_entregues bigint, docs_vencendo bigint, docs_vencidos bigint, valor_pendente numeric, valor_vencido numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    ...
    SELECT 
      ...
      -- Docs vencendo (próximos 7 dias) - do cliente
      COALESCE((
        SELECT COUNT(*) 
        FROM documentos_financeiros df 
