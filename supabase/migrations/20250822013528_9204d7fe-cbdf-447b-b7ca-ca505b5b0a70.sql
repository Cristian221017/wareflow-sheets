...
-- Limpar dados existentes de teste (apenas em dev)
DELETE FROM notas_fiscais WHERE numero_nf LIKE 'DEV-%';

-- Inserir transportadora de exemplo se não existir
...
SELECT 
  ...
  '(11) 99999-9999'
WHERE NOT EXISTS (SELECT 1 FROM transportadoras WHERE id = '00000000-0000-4000-8000-000000000001');
...
SELECT 
  ...
  '(11) 88888-8888',
  '00000000-0000-4000-8000-000000000001'
WHERE NOT EXISTS (SELECT 1 FROM clientes WHERE id = '00000000-0000-4000-8000-000000000002');

-- Inserir ~10 NFs distribuídas nos 3 status
