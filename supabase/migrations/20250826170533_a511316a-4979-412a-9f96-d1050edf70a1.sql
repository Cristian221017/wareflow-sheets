-- Verificar e corrigir políticas RLS para system_logs
...
-- Verificar e corrigir políticas RLS para event_log  
DROP POLICY IF EXISTS "Super admins can view all event logs" ON event_log;
DROP POLICY IF EXISTS "Users can view event logs from their transportadora" ON event_log;
DROP POLICY IF EXISTS "System can insert event logs" ON event_log;

-- Políticas para event_log (compatibilidade)
CREATE POLICY "Super admins can view all event logs" ON event_log
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM user_transportadoras ut
      WHERE ut.user_id = auth.uid() 
...
CREATE POLICY "Users can view event logs from their transportadora" ON event_log
  FOR SELECT USING (
    transportadora_id = (
      SELECT ut.transportadora_id
      FROM user_transportadoras ut
