-- Adicionar campo status_separacao na tabela notas_fiscais
ALTER TABLE public.notas_fiscais 
ADD COLUMN status_separacao TEXT DEFAULT 'pendente';

-- Criar tipo enum para os status de separação (opcional, mas mais organizado)
...
ALTER TABLE public.notas_fiscais 
ALTER COLUMN status_separacao TYPE separacao_status USING status_separacao::separacao_status;

-- Adicionar constraint para garantir que status_separacao só seja usado quando NF está armazenada
-- (via trigger, já que CHECK constraints não podem usar funções)
...
COMMENT ON COLUMN public.notas_fiscais.status_separacao IS 'Status da separação da mercadoria: pendente, em_separacao, separacao_concluida, separacao_com_pendencia';
