-- Atualizar função get_current_user_dashboard para corrigir lógica de documentos vencidos
...
BEGIN
  ...
  IF v_transportadora_id IS NOT NULL THEN
    ...
    SELECT 
      ...
      -- NFs armazenadas (excluindo as que já estão em viagem ou entregues)
      COALESCE((
        SELECT COUNT(*) 
        FROM notas_fiscais nf 
        WHERE nf.transportadora_id = v_transportadora_id 
        AND nf.status = 'ARMAZENADA'
        AND COALESCE(nf.status_separacao, 'pendente') NOT IN ('em_viagem', 'entregue')
      ), 0) as nfs_armazenadas,
      -- NFs confirmadas (mas não entregues nem em viagem)
      COALESCE((
        SELECT COUNT(*) 
        FROM notas_fiscais nf 
        WHERE nf.transportadora_id = v_transportadora_id 
        AND nf.status = 'CONFIRMADA'
        AND COALESCE(nf.status_separacao, 'pendente') NOT IN ('entregue', 'em_viagem')
      ...
      -- Docs vencendo (próximos 7 dias) - apenas em aberto e não vencidos
      COALESCE((
        SELECT COUNT(*) 
        ...
        AND df.data_vencimento >= CURRENT_DATE
      ), 0) as docs_vencendo,
      -- Docs vencidos - CORRIGIDO: status 'Vencido' OU 'Em aberto' com data vencida
      COALESCE((
        SELECT COUNT(*) 
        FROM documentos_financeiros df 
        WHERE df.transportadora_id = v_transportadora_id 
        AND (
          df.status = 'Vencido' 
          OR (df.status = 'Em aberto' AND df.data_vencimento < CURRENT_DATE)
        )
      ), 0) as docs_vencidos,
      -- Valor pendente - apenas em aberto e não vencidos
      COALESCE((
        SELECT SUM(valor) 
        FROM documentos_financeiros df 
        WHERE df.transportadora_id = v_transportadora_id 
      ...
      -- Valor vencido - CORRIGIDO: status 'Vencido' OU 'Em aberto' com data vencida
      COALESCE((
        SELECT SUM(valor) 
        FROM documentos_financeiros df 
        WHERE df.transportadora_id = v_transportadora_id 
  ...
  ELSE
    ...
    SELECT 
      ...
      -- NFs armazenadas do cliente (excluindo as que já estão em viagem ou entregues)
      COALESCE((
        SELECT COUNT(*) 
        FROM notas_fiscais nf 
        WHERE nf.cliente_id = v_cliente_id 
      ...
      -- NFs confirmadas do cliente (mas não entregues nem em viagem)
      COALESCE((
        SELECT COUNT(*) 
        FROM notas_fiscais nf 
        WHERE nf.cliente_id = v_cliente_id 
