-- Remover funções duplicadas e criar versão definitiva
DROP FUNCTION IF EXISTS public.nf_update_status_separacao(uuid, text, text);
DROP FUNCTION IF EXISTS public.nf_update_status_separacao(uuid, public.separacao_status, text);

-- Criar função definitiva que aceita text e converte internamente
...
BEGIN
  ...
  -- Atualizar status de separação (cast para o tipo enum)
