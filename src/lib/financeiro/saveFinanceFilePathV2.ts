...
export async function saveFinanceFilePathV2(docId: string, kind: FileKind, uploadPath: string) {
  ...
  const { data, error } = await supabase.rpc("set_financeiro_file_path_v2" as any, {
    p_doc_id: docId,
    p_kind: kind,
    p_path: uploadPath
  });
